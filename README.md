...
| **Docker** _(optional)_ | Only needed for containerised setup (Option B).                                                          |
...
- **Measures quality with 7 metrics** — ROUGE-L, semantic similarity, context-question similarity, context-GT similarity, retrieval precision@K, context sufficiency, and hallucination rate — all embedding-based (no extra LLM calls)
...
```bash
cp env_format.txt .env    # env_format.txt is a complete template with all variables and default values
```
...
| `OLLAMA_BASE_URL`    | `http://localhost:11434` | Use `http://host.docker.internal:11434` for Docker setup                           |
