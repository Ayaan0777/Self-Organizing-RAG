...
from db.models import QueryLog, LowRecallEvent, RepairReport, EvalSnapshot
...
page = st.sidebar.radio(
    "📋 Navigate",
    ["Overview", "Query Diagnostics", "Low-Recall Events", "Repair History", "Eval History"],
)
...
if page == "Overview":
    ...
    repairs  = session.query(RepairReport).count()
    ...
    | `score_drop` | Gap between rank-1 and rank-3 too large | **0.3** |
                ...
                if event:
                    ...
                    for d in detectors:
                        explanations = {
                            ...
                            "score_drop": "🟡 **Score Drop** — Large gap between rank-1 and rank-3 scores",
                            "llm_uncertainty": "🟠 **LLM Uncertainty** — Response contains hedging language",
                            "short_answer": "🔵 **Short Answer** — Response is under 50 characters",
                        }
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    st.subheader("🔧 Repair Reports")

    rows = session.query(RepairReport).order_by(
               RepairReport.timestamp.desc()).limit(100).all()

    if not rows:
        st.info("No repairs have been run yet.")
    else:
        data = [{
            "ID": r.id,
            "Event": r.event_id,
            "Strategy": r.strategy_used,
            "Score Before": r.score_before,
            "Score After": r.score_after,
            "Δ Score": round((r.score_after or 0) - (r.score_before or 0), 4),
            "Chunks Before": r.chunks_before,
            "Chunks After": r.chunks_after,
        ...
        if not df.empty and "Δ Score" in df.columns:
            st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
            st.subheader("📊 Score Improvement per Repair")
            st.bar_chart(df.set_index("ID")["Δ Score"], height=250)
